...
export class BettingDataService {
  ...
  private processRealOddsData(game: any, oddsData: any[]): BettingOpportunity[] {
    ...
    // Create comprehensive betting opportunities with side-by-side odds comparison
    const comprehensiveOpportunities = this.createComprehensiveBettingOpportunities(game, oddsData);
    opportunities.push(...comprehensiveOpportunities);
  ...
  // Create comprehensive betting opportunities with side-by-side odds comparison
  private createComprehensiveBettingOpportunities(game: any, oddsData: any[]): BettingOpportunity[] {
    const opportunities: BettingOpportunity[] = [];
    const gameTitle = this.formatGameTitle(game);
    ...
    const spreadBooks = oddsData.filter(book => book.spread && book.spreadLine1 && book.spreadLine2);
    const totalBooks = oddsData.filter(book => book.overUnder && book.overUnderLineOver && book.overUnderLineUnder);

    // Create moneyline opportunity with all available books
    ...
    // Create spread opportunities with side-by-side comparison
    ...
    // Create total opportunities with side-by-side comparison
